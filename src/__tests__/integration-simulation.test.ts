...
describe("End-to-End Simulation Test Data Validation", () => {
  ...
  it("Scenario Alpha: nominal operations simulation", () => {
    ...
    console.log(
      "Sanitization Injection Alert:",
      result.hadInjection ? "YES" : "NO",
    );
    console.log("Validation Errors:", result.validationErrors);
    console.log(
      "Evaluated System Status:",
      result.evaluation.system_status.toUpperCase(),
    );
  ...
  it("Scenario Beta: crowd density bottleneck simulation", () => {
    ...
    expect(result.evaluation.reasoning_chain).toContain(
      "Zona ZONE-B3 alcanzó el 88%",
    );
  });
  ...
  it("Scenario Incident: security incident alarm simulation", () => {
    ...
    expect(result.evaluation.reasoning_chain).toContain(
      "incident critique a été signalé",
    );
  });
