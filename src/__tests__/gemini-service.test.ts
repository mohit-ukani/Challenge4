...
describe("Gemini AI Service Client Suite", () => {
  ...
  it("should successfully parse a valid JSON response from Gemini API", async () => {
            ...
            parts: [
              {
                text: JSON.stringify({
                  briefing:
                    "Crowd density is elevated at Zone T1. Recommended warning guidelines applied.",
                  riskLevel: "alert",
    ...
    const result = await generateSituationalAnalysis(
      telemetry,
      context,
      "alert",
      "en",
  ...
  it("should throw an error when API key is missing", async () => {
    ...
    await expect(
      generateSituationalAnalysis(telemetry, context, "alert", "en"),
