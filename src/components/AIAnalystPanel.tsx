...
export const AIAnalystPanel: React.FC<AIAnalystPanelProps> = React.memo(({
  ...
  const statusAccent =
    systemStatus === 'alert' ? 'var(--accent-danger)' :
    systemStatus === 'warning' ? 'var(--accent-warning)' :
    'var(--accent)';
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
          ...
          <button
            ...
            aria-label={analysisState === 'loading' ? t('ai.analyzing') : t('ai.reanalyze')}
            style={{
              padding: 'var(--space-1) var(--space-3)',
              backgroundColor: analysisState === 'loading' ? 'var(--bg-surface-elevated)' : 'transparent',
              border: `1px solid ${statusAccent}`,
              borderRadius: 'var(--radius)',
              color: analysisState === 'loading' ? 'var(--text-tertiary)' : statusAccent,
          ...
          >
            {analysisState === 'loading' ? t('ai.analyzing') : t('ai.reanalyze')}
          </button>
        </div>
      </div>
      ...
      >
        ...
        {analysisState === 'loading' && (
          <div style={{
            ...
            {/* Skeleton lines */}
            <div className="ai-skeleton" style={{ width: '100%', height: '14px' }} />
            <div className="ai-skeleton" style={{ width: '80%', height: '14px' }} />
            <div className="ai-skeleton" style={{ width: '90%', height: '14px' }} />
          </div>
        )}
        ...
        {analysisState === 'error' && errorMessage && (
          ...
          >
            <strong style={{ color: 'var(--accent-danger)', display: 'block', marginBottom: 'var(--space-1)' }}>
              {t('ai.error_title')}
            </strong>
            ...
            {errorMessage.includes('VITE_GEMINI_API_KEY') && (
              <p style={{ color: 'var(--text-tertiary)', marginTop: 'var(--space-2)', fontSize: '0.8rem' }}>
                {t('ai.key_hint')}
              </p>
            )}
          </div>
