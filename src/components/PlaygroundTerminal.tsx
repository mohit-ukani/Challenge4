...
interface PlaygroundTerminalProps {
  ...
  onInjectDirectState: (telemetry: TelemetryPayload, context: ContextPayload) => void;
  onReset: () => void;
...
export const PlaygroundTerminal: React.FC<PlaygroundTerminalProps> = React.memo(({
  ...
  return (
    <aside className="aside-panel" aria-labelledby="terminal-title">
      <h2 id="terminal-title" style={{ fontSize: '1.2rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
        {t('ui.terminal_title')}
      </h2>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
        <label htmlFor="raw-json-textarea" style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
          {t('ui.raw_json')}
        </label>
        <textarea
      ...
      >
        ...
        <div style={{ maxHeight: '150px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '4px' }}>
          {validationPassed === true && (
            <span style={{ color: 'var(--accent)' }}>✓ {t('ui.validator_success')}</span>
          )}
          {validationErrors.map((err, i) => (
            <span key={i} style={{ color: 'var(--accent-warning)', display: 'block' }}>
              ! {err}
            </span>
          ))}
