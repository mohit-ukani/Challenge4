...
export function validatePayload(parsedJson: unknown): ValidationResult {
  ...
  if ('zone_id' in raw) {
    ...
    errors.push("Missing required field 'zone_id', using fallback default 'UNKNOWN'.");
  }
  ...
  if ('current_crowd_density' in raw) {
    ...
    if (rawDensity === null || rawDensity === undefined) {
      ...
      if (Number.isNaN(parsedDensity) || !Number.isFinite(parsedDensity)) {
        errors.push(`Malformed 'current_crowd_density' (${rawDensity}) coerced to 0.`);
      } else {
        // Range clamping
        if (parsedDensity < 0) {
          telemetry.current_crowd_density = 0;
          errors.push(`Crowd density (${parsedDensity}%) clamped to minimum of 0%.`);
        } else if (parsedDensity > 100) {
          telemetry.current_crowd_density = 100;
          errors.push(`Crowd density (${parsedDensity}%) clamped to maximum of 100%.`);
        } else {
    ...
    errors.push("Missing required field 'current_crowd_density', defaulting to 0.");
  }
  ...
  if ('ambient_noise_levels' in raw) {
    ...
    if (rawNoise === null || rawNoise === undefined) {
      ...
      if (Number.isNaN(parsedNoise) || !Number.isFinite(parsedNoise)) {
        errors.push(`Malformed 'ambient_noise_levels' (${rawNoise}) coerced to 0 dB.`);
      } else {
        if (parsedNoise < 0) {
          telemetry.ambient_noise_levels = 0;
          errors.push(`Ambient noise (${parsedNoise} dB) clamped to minimum of 0 dB.`);
        } else if (parsedNoise > 200) {
          telemetry.ambient_noise_levels = 200;
          errors.push(`Ambient noise (${parsedNoise} dB) clamped to maximum of 200 dB.`);
        } else {
    ...
    errors.push("Missing required field 'ambient_noise_levels', defaulting to 0 dB.");
  }
  ...
  if ('incident_flag' in raw) {
    ...
    if (rawFlag === null || rawFlag === undefined) {
      ...
      if (typeof rawFlag !== 'boolean') {
        errors.push(`Coerced non-boolean 'incident_flag' (${rawFlag}) to ${telemetry.incident_flag}.`);
      }
    }
  } else {
  ...
  if ('local_time' in raw) {
    ...
    if (timeFormatRegex.test(rawTime)) {
      ...
      errors.push(`Invalid 'local_time' format (${rawTime}), fallback to current time: ${context.local_time}.`);
    }
  } else {
    errors.push(`Missing 'local_time' field, defaulting to current system time: ${context.local_time}.`);
  }
