...
const dictionaries: Record<Locale, Translations> = {
  en: {
    ...
    "ui.xss_alert":
      "SECURITY CRITICAL ALERT: Malicious markup or script tags detected in payload. Neutralization protocols applied. Scripts stripped to prevent Cross-Site Scripting (XSS).",
    ...
    "ai.subtitle":
      "Powered by Google Gemini · Auto-updates on status transitions",
    ...
    "ai.key_hint":
      "Add VITE_GEMINI_API_KEY to a .env file and restart the dev server.",
    "ai.waiting":
      "Waiting for first telemetry injection to trigger analysis...",
    "ai.actions_header": "⚡ Recommended Actions",
    "ai.tips_header": "👥 Crowd Management",

    // XAI templates
    "reasoning.density_alert":
      "At {time}, Zone {zone} density hit {density}%, triggering bottleneck alert protocols.",
    "reasoning.incident_flag":
      "At {time}, a critical incident was flagged in Zone {zone}, initiating immediate security response.",
    "reasoning.density_warning":
      "At {time}, Zone {zone} density reached {density}%, requiring crowd control monitoring.",
    "reasoning.noise_warning":
      "At {time}, ambient noise levels in Zone {zone} reached {noise} dB, triggering loud crowd advisory.",
    "reasoning.nominal":
      "At {time}, Zone {zone} density operates at {density}% and ambient noise at {noise} dB. All systems nominal.",
  },
  es: {
    // Header & Info
    ...
    "app.role":
      "Tablero del Director de Seguridad del Local y Líder de Voluntarios",
    ...
    "ui.xss_alert":
      "ALERTA CRÍTICA DE SEGURIDAD: Se detectaron etiquetas de script o marcado malicioso en la carga útil. Protocolos de neutralización aplicados. Scripts eliminados para evitar Cross-Site Scripting (XSS).",
    ...
    "ai.subtitle":
      "Desarrollado por Google Gemini · Actualizaciones automáticas en transiciones de estado",
    ...
    "ai.key_hint":
      "Agregue VITE_GEMINI_API_KEY a un archivo .env y reinicie el servidor de desarrollo.",
    "ai.waiting":
      "Esperando la primera inyección de telemetría para activar el análisis...",
    "ai.actions_header": "⚡ Acciones recomendadas",
    "ai.tips_header": "👥 Gestión de multitudes",

    // XAI templates
    "reasoning.density_alert":
      "A las {time}, la densidad de la Zona {zone} alcanzó el {density}%, activando los protocolos de alerta por embotellamiento.",
    "reasoning.incident_flag":
      "A las {time}, se reportó un incidente crítico en la Zona {zone}, iniciando una respuesta de seguridad inmediata.",
    "reasoning.density_warning":
      "A las {time}, la densidad de la Zona {zone} llegó al {density}%, requiriendo monitoreo de control de multitudes.",
    "reasoning.noise_warning":
      "A las {time}, los niveles de ruido ambiental en la Zona {zone} alcanzaron {noise} dB, activando un aviso de multitud ruidosa.",
    "reasoning.nominal":
      "A las {time}, la Zona {zone} opera con una densidad del {density}% y ruido ambiental de {noise} dB. Todos los sistemas nominales.",
  },
  fr: {
    // Header & Info
    ...
    "app.role":
      "Tableau de Bord du Chef de la Sécurité du Site & Chef des Bénévoles",
    ...
    "ui.validator_success":
      "La validation de la charge utile a réussi proprement.",
    ...
    "ui.xss_alert":
      "ALERTE DE SÉCURITÉ CRITIQUE : Balises de script ou balises malveillantes détectées dans la charge utile. Protocoles de neutralisation appliqués. Scripts supprimés pour empêcher le Cross-Site Scripting (XSS).",
    ...
    "ai.subtitle":
      "Propulsé par Google Gemini · Mises à jour automatiques sur les transitions d'état",
    ...
    "ai.key_hint":
      "Ajoutez VITE_GEMINI_API_KEY dans un fichier .env et redémarrez le serveur.",
    "ai.waiting":
      "En attente de la première injection de télémétrie pour déclencher l'analyse...",
    "ai.actions_header": "⚡ Actions recommandées",
    "ai.tips_header": "👥 Gestion des foules",

    // XAI templates
    "reasoning.density_alert":
      "À {time}, la densité de la Zone {zone} a atteint {density}%, déclenchant les protocoles d'alerte de goulot d'étranglement.",
    "reasoning.incident_flag":
      "À {time}, un incident critique a été signalé dans la Zone {zone}, initiant une réponse de sécurité immédiate.",
    "reasoning.density_warning":
      "À {time}, la densité de la Zone {zone} a atteint {density}%, nécessitant une surveillance du contrôle des foules.",
    "reasoning.noise_warning":
      "À {time}, les niveaux de bruit ambiant dans la Zone {zone} ont atteint {noise} dB, déclenchant un avis de foule bruyante.",
    "reasoning.nominal":
      "À {time}, la Zone {zone} fonctionne avec une densité de {density}% et un bruit ambiant de {noise} dB. Tous les systèmes nominaux.",
  },
  ar: {
    // Header & Info
    ...
    "ui.xss_alert":
      "تنبيه أمني هام: تم اكتشاف تعليمات برمجية أو علامات برمجية ضارة في الحمولة. تم تطبيق بروتوكولات المعالجة وتجريد النصوص البرمجية لمنع هجمات XSS.",
    ...
    "ai.key_hint":
      "أضف VITE_GEMINI_API_KEY في ملف .env ثم أعد تشغيل خادم التطوير.",
    ...
    // XAI templates
    "reasoning.density_alert":
      "في الساعة {time}، وصلت كثافة المنطقة {zone} إلى {density}%، مما أدى إلى تفعيل بروتوكولات تنبيه الاختناق.",
    "reasoning.incident_flag":
      "في الساعة {time}، تم الإبلاغ عن حادث خطير في المنطقة {zone}، مما أدى إلى بدء استجابة أمنية فورية.",
    "reasoning.density_warning":
      "في الساعة {time}، وصلت كثافة المنطقة {zone} إلى {density}%، مما يتطلب مراقبة التحكم في الحشود.",
    "reasoning.noise_warning":
      "في الساعة {time}، وصلت مستويات الضوضاء المحيطة في المنطقة {zone} إلى {noise} ديسيبل، مما أدى إلى إطلاق تنبيه حشود صاخبة.",
    "reasoning.nominal":
      "في الساعة {time}، تعمل المنطقة {zone} بكثافة {density}% وضوضاء محيطة تبلغ {noise} ديسيبل. جميع الأنظمة تعمل بشكل طبيعي.",
  },
};
